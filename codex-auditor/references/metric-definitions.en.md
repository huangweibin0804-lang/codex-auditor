...
`event_coverage_minutes` counts distinct local calendar minutes containing a user or visible assistant message. `interaction_interval_estimate_minutes` unions only these intervals: user-to-next-assistant, capped at two minutes, and assistant-to-next-user, capped at fifteen minutes. Assistant-to-assistant and tool events do not extend activity.

Preset ranges use local natural days and include today: one day starts at today's local midnight, seven days starts at local midnight six dates ago, and thirty days starts at local midnight twenty-nine dates ago. Custom dates use the local timezone as well.
...
- `unsupported_fact_count`: specific factual claims without valid evidence IDs; target 0 in every run.
