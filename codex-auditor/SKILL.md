...
When changing the local HTML renderer, read [references/report-design-system.md](references/report-design-system.md). The renderer carries the background as inline SVG and CSS, so a normal Skill installation produces the same background without downloading external assets.

If the user asks for “只做本地统计” or “不审查对话正文”, run the same `start` command with `--mode local` and omit semantic stages.
...
Read [references/semantic-audit-input.md](references/semantic-audit-input.md) and [references/semantic-audit-output.md](references/semantic-audit-output.md) only when a semantic stage needs repair. Read [references/metric-definitions.md](references/metric-definitions.md) when interpreting estimates or rules.
